...
export class WikiShield {
	constructor(wikishieldData) {
			...
			if (base) {
				...
				while (allScripts.length > 0) {
					...
					const willBeRun = (current.name && current.name === "if"
						&& this.wikishieldEventData.conditions[current.condition].check(this.queue.currentEdit)) || !current.name;
		...
		const ifAndTrue = script.name && script.name === "if"
			&& this.wikishieldEventData.conditions[script.condition].check(currentEdit);
