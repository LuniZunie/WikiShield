...
export class WikiShieldInterface {
	...
	async build() {
		...
		async newEditSelected(edit) {
			...
			this.elem("#middle-top").innerHTML = `
				<div style="display: flex; overflow: auto hidden; white-space: nowrap">
					...
					<div>
						...
						<span style="color: ${this.wikishield.util.getChangeColor(edit.sizediff || 0)}">${this.wikishield.util.getChangeString(edit.sizediff || 0)}</span>
					</div>
				</div>
			...
			$rightTop.innerHTML = `
				...
				<div>
					<span class="fa fa-pencil"></span>
					<span id="consecutive-sizediff" style="color: ${this.wikishield.util.getChangeColor(0)}">-</span>
				</div>
			`;
			...
			edit.consecutive.then((object) => {
				...
				$consecutiveTime.innerHTML = this.wikishield.formatNotificationTimeShort(new Date(timestamp));
				...
				$sizediff.style.color = this.wikishield.util.getChangeColor(sizediff);
				$sizediff.innerHTML = this.wikishield.util.getChangeString(edit.sizediff || 0);
			});
	...
	class WikiShieldProgressBar {
		constructor() {
			...
			this.wikishield.interface.elem("#progress-bar-container").appendChild(this.element);
	...
	class WikiShield {
		constructor() {
			...
			// Initialize queue - will be set after this.wikishield global is assigned
			this.queue = null;
		...
		/**
		 * Create the interface for checking if the user is allowed to use this.wikishield
		 */
		...
		async warnUser(user, warningObject, warnLevel, articleName, revid) {
			...
			if (!userTalkContent.match("== ?" + this.wikishield.util.monthSectionName() + " ?==")) {
				userTalkContent += `\n== ${this.wikishield.util.monthSectionName()} ==\n`;
			}

			const sections = userTalkContent.split(/(?=== ?[\w\d ]+ ?==)/g);

			for (let section in sections) {
				if (sections[section].match(new RegExp("== ?" + this.wikishield.util.monthSectionName() + " ?=="))) {
		...
		// Method incomplete - this was extracted from a larger method
